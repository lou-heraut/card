...
def _meta_rows(card) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "statistic_en": cfield(en, "statistic"),
        ...
        "statistic_fr": cfield(fr, "statistic"),
...
def extract(data, cards=("QA", "QJXA"), path=None,
    ...
    data : pandas.DataFrame
        ...
        `card.list_cards` or `card.info`.
    cards : sequence of str, default ("QA", "QJXA")
        Names of the cards to run. The `card` column of
        `card.list_cards` holds them: one card can produce several
    ...
    dict
        ...
        another extraction, or into `card.trend`.
