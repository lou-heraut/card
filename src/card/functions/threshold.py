...
def apply_threshold(X, lim, where="<=", what="X", select="all",
    ...
    """Analyse the episodes where X crosses a threshold.

    An episode is a contiguous run of indices satisfying the condition.

    Parameters
    ----------
    X : array-like
        The series to examine.
    lim : float or array-like
        The threshold, a scalar or a column.
    where : {"<=", "<", ">=", ">"}, default "<="
    ...
    period : list, optional
        Pair of bounds restricting the analysis.
...
def deficit_volume(Q, threshold):
    """Deficit volume below a threshold, over the longest continuous episode.
    ...
    threshold : float or array-like
        The threshold below which the deficit is counted.

    Returns
    -------
    float
        The volume in hm³, the sum of the discharges below the threshold
        multiplied by 86400 and divided by 10⁶.
    """
