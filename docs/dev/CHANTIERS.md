...
## 10. card-api : consultation des fiches, du chemin serveur au lien citable

Objectif de fond, formulé par l'utilisateur le 2026-07-20 : faire de la
consultation des fiches par l'API la partie la plus agréable du service.
Aider quelqu'un à comprendre ce que fait une fiche et à retrouver les
...
machine qui exécute (`extraction.py:222`, reversé tel quel par
`main.py:98-101` ; `clean()` ne fait que NaN -> null). En production
cela donne
...
Le champ reste utile en interne (`trend.py:32`, `main.py:49-51`), donc
il s'agit de ne pas le sortir, pas de le supprimer de `card`.

`/v1/cards/{id}` construit bien un lien GitHub, mais en cherchant la
sous-chaîne `src/card/cards/` dans le chemin (`main.py:115-118`). Cette
sous-chaîne n'existe qu'en install éditable. En production la condition
est fausse et le lien disparaît en silence, vérifié. D'où la situation
actuelle : le chemin interne là où il ne sert à rien, pas de lien
public là où il servirait.
...
employées lisibles depuis le catalogue (aujourd'hui la colonne
`functions` est une liste de noms nus, sans dire ce que fait chacun),
et donner à lire la chaîne de traitement d'une fiche. À cadrer avec le
chantier 5 (documentation utilisateur) plutôt qu'en isolé.
