...
def test_defaults_applied():
    ...
    assert gl["to_normalise"] is True
...
def test_adaptive_sampling_parsed():
    ...
    assert sp["funct"]["fn_name"] == "nanmax"
    assert sp["funct"]["cols"] == ["QMA"]


def test_is_date_flag():
    card = load_card(path_of("tQJXA"))
    entry = card["processes"][0]["funct"][0]
...
def test_positional_literal():
    ...
    entries = [e for p in card["processes"] for e in p["funct"]
