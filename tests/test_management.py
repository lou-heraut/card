...
def test_management_copies_with_ids(tmp_path):
    dest = tmp_path / "WIP"
    CARD_management(CARD_name={"analyse": ["QA", "QMNA"]}, CARD_path=dest)
...
def test_management_refuses_overwrite(tmp_path):
    dest = tmp_path / "WIP"
    CARD_management(CARD_name=["QA"], CARD_path=dest)
    with pytest.raises(FileExistsError):
        CARD_management(CARD_name=["QA"], CARD_path=dest)
    CARD_management(CARD_name=["QA"], CARD_path=dest, overwrite=True)
